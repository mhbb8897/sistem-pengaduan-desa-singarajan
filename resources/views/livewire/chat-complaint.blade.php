<div class="space-y-4" wire:poll.5s>
    ...
    <div class="max-h-64 overflow-y-auto space-y-3 p-2 bg-gray-900 rounded-lg">
        @foreach($messages as $msg)
            <div class="p-3 rounded-lg {{ $msg->sender_role === 'admin' ? 'bg-gray-800 border-l-4 border-yellow-500' : 'bg-gray-700' }}">
    ...
    <div class="flex gap-2">
        <input type="text" wire:model="message" 
            ...
            class="flex-1 !text-gray-900  !text-black rounded-lg text-sm focus:ring-yellow-500">
        <button wire:click="sendMessage" 
            class="px-4 py-2 bg-yellow-600 hover:bg-yellow-500 text-white rounded-lg text-sm font-bold">
            Kirim
        </button>
    </div>
</div>
